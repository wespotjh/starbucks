"""
=============================================================================
 zen_day6_final.py — 젠제네틱스 데이팩(6포) 최종 실행판
=============================================================================

 ★ 아무것도 고치지 않아도 됩니다. 열어서 ▶ 만 누르세요.
   두 가지 시퀀스를 자동으로 렌더합니다:
     render/potassium_day6_turn/  닫힌 상태 턴테이블 36장
     render/potassium_day6_open/  뚜껑 0→140도 개봉 24장 (스크롤 스크럽용)
...
PRODUCT = "potassium"
SCRIPT_VERSION = "day6-final"
TURN_FRAMES = 36     # 턴테이블 장수
OPEN_FRAMES = 24     # 개봉 시퀀스 장수 (0 → 140도)
...
def set_lid_angle(obj, base_coords, deg):
    """힌지(뒤 상단 모서리, x축) 기준으로 뚜껑을 절대 각도로 젖힌다.
    base_coords: 닫힌 상태의 원본 좌표. 매 프레임 원본에서 다시 계산한다."""
    a = -math.radians(deg)
    cy, cz = Y_B, Z_HINGE
    me = obj.data
...
def main():
    ...
    def shot(path, rot_z_deg):
        ...
        if os.path.exists(path):
            print(f"  [{done:>3}/{total}] 이미 있음, 건너뜀")
            return
        for o in parts:
        ...
        print(f"  [{done:>3}/{total}]  경과 {el/60:.1f}분  남은시간 약 {eta/60:.1f}분")

    # 1) 닫힌 상태 턴테이블
    d = os.path.join(OUT_DIR, f"{PRODUCT}_day6_turn")
    ...
    # 2) 개봉 시퀀스 (앞 3/4 고정, 카메라 높임, 완급 곡선)
    d = os.path.join(OUT_DIR, f"{PRODUCT}_day6_open")
    os.makedirs(d, exist_ok=True)
    print(f"\n  [2/2] 개봉 시퀀스 {OPEN_FRAMES}장 → {d}")
    aim_camera(cam, tgt, size, high=True)
    for i in range(OPEN_FRAMES):
        t = i / (OPEN_FRAMES - 1)
        ease = t * t * (3 - 2 * t)          # smoothstep: 시작·끝 완만
        set_lid_angle(lid, lid_base, LID_OPEN_DEG * ease)
        shot(os.path.join(d, f"frame_{i+1:04d}.png"), 25)

    print("\n  전부 완료!")
    print(f"  턴테이블 : render/{PRODUCT}_day6_turn/  ({TURN_FRAMES}장)")
    print(f"  개봉     : render/{PRODUCT}_day6_open/  ({OPEN_FRAMES}장)\n")
