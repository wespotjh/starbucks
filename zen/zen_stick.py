"""
=============================================================================
 zen_stick.py — 젠제네틱스 스틱포 지오메트리 + 턴테이블 렌더
=============================================================================

 실행 방법  (경로 입력 없음)
   1. 이 파일을 zen 폴더 안에 넣습니다
      zen/zen_stick.py, zen/textures/, zen/render/
   2. Blender 실행
   3. 상단 탭에서 스크립팅함(Scripting) 클릭
   4. 폴더 아이콘(열기)으로 zen_stick.py 를 직접 엽니다
...
# --- 품질 프리셋 -------------------------------------------------------------
# "CHECK"  단일 프레임, 빠르게. 처음에는 반드시 이걸로 확인하세요.
# "TURN"   턴테이블 60프레임. CHECK 통과 후에 돌리세요.
MODE = "CHECK"
...
#           따로 두면 글자가 늘어나는 사고가 나므로 하나로 묶었습니다.
#           1보다 크면 그래픽이 작아지며 가운데로 모입니다.
#           1.10 이면 UV 가 0.045~0.955 범위라 텍스처 밖으로 넘치지 않습니다.
...
def main():
    ...
    if cfg["frames"] == 1:
        ...
        print("\n  [확인 요령]")
        print("   배경은 투명이라 미리보기에서 흰색으로 보일 수 있습니다.")
        print("   어두운 바탕에 얹어서 보세요.")
        print("   글자가 안 보이고 통째로 하얗다  → LIGHT_POWER 를 0.5 로")
        ...
        # 프레임마다 직접 회전시켜 한 장씩 렌더하면 버전 문제가 없고,
        # 중간에 끊겨도 이어서 돌릴 수 있습니다.
        n = cfg["frames"]
        ...
        for i in range(n):
            ...
            eta = el / done * (n - done)
            print(f"  [{done:>3}/{n}]  경과 {el/60:.1f}분  남은시간 약 {eta/60:.1f}분")

        print(f"\n  완료: {d}")
        print(f"  총 {n}장 / {(time.time()-t0)/60:.1f}분\n")
